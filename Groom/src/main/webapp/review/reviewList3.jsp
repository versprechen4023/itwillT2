...
<!-- 리뷰 분류 -->
		...
		<a href="reviewList3.re?pro_name=부분+목욕" class="review-active">부분+목욕</a>
...
<div class="prev-next" data-animate-effect="fadeInLeft">
    <% if (currentPage > 1) { %>
        <a href="reviewList3.re?pro_name=<%= request.getParameter("pro_name") %>&page=<%= currentPage - 1 %>" class="pgL"><span class="m-tcol-c">&lt;</span></a>
    <% } %>
    <% 
    ...
    for (int i = startPage; i <= endPage; i++) { %>
        <a href="reviewList3.re?pro_name=<%= request.getParameter("pro_name") %>&page=<%= i %>" <%= (i == currentPage) ? "class='review-active'" : "" %>><%= i %></a>
    <% } %>
    <% if (currentPage < totalPages) { %>
        <a href="reviewList3.re?pro_name=<%= request.getParameter("pro_name") %>&page=<%= currentPage + 1 %>" class="pgR"><span class="m-tcol-c">&gt;</span></a>
    <% } %>
</div>
