...
<%
//reviewList가 비어있지 않은지 확인
if (reviewList != null && !reviewList.isEmpty()) {
 ReviewDTO reviewDTO2 = reviewList.get(0); // 첫 번째 ReviewDTO 객체를 가져옴
 int totalReview = reviewDTO2.getTotal_review();
%>
		<a href="reviewList.re">전체 <%=reviewDTO2.getTotal_review() %></a>
<%
} else {
%>
		<a href="reviewList.re">전체 0</a>
<%
}
%>
		<a href="reviewList1.re?pro_name=목욕">목욕</a>
		<a href="reviewList2.re?pro_name=부분미용" class="review-active">부분미용</a>
...
<div class="prev-next" data-animate-effect="fadeInLeft">
    <% if (currentPage > 1) { %>
        <a href="reviewList2.re?pro_name=<%= request.getParameter("pro_name") %>&page=<%= currentPage - 1 %>" class="pgL"><span class="m-tcol-c">&lt;</span></a>
    <% } %>
    <% 
    ...
    for (int i = startPage; i <= endPage; i++) { %>
        <a href="reviewList2.re?pro_name=<%= request.getParameter("pro_name") %>&page=<%= i %>" <%= (i == currentPage) ? "class='review-active'" : "" %>><%= i %></a>
    <% } %>
    <% if (currentPage < totalPages) { %>
        <a href="reviewList2.re?pro_name=<%= request.getParameter("pro_name") %>&page=<%= currentPage + 1 %>" class="pgR"><span class="m-tcol-c">&gt;</span></a>
    <% }
