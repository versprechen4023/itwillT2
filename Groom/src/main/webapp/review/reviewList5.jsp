...
<div class="prev-next" data-animate-effect="fadeInLeft">
    <% if (currentPage > 1) { %>
        <a href="reviewList5.re?pro_name=<%= request.getParameter("pro_name") %>&page=<%= currentPage - 1 %>" class="pgL"><span class="m-tcol-c">&lt;</span></a>
    <% } %>
    <% 
    ...
    for (int i = startPage; i <= endPage; i++) { %>
        <a href="reviewList5.re?pro_name=<%= request.getParameter("pro_name") %>&page=<%= i %>" <%= (i == currentPage) ? "class='review-active'" : "" %>><%= i %></a>
    <% } %>
